  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="flex-1">Все статьи</span>
      </button>

      {/* Categories list */}
      <div className="flex-1 space-y-0.5 overflow-y-auto">
        {isLoading ? (
          <div className="px-2 py-4 text-center text-[12px] text-[#94a3b8]">Загрузка...</div>
        ) : rootCategories.length === 0 ? (
          <div className="px-2 py-4 text-center text-[12px] text-[#94a3b8]">Нет категорий</div>
        ) : (
      ...
      {isAdmin && (
        <>
          ...
          <button
            ...
            className="flex w-full items-center gap-1.5 rounded-lg px-2 py-1.5 text-[13px] text-[#6366f1] transition-colors hover:bg-[#eef2ff] disabled:opacity-50"
          >
            <Plus className="h-3.5 w-3.5" />
            <span>{createCategory.isPending ? 'Создание...' : '+ Добавить категорию'}</span>
          </button>
        </>
      )}
    </aside>
